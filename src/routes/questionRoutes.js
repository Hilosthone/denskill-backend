 ...
 * 
 * TEACHING NOTE:
 * Route ordering is critical in Express. Notice that static/specific path segments 
 * (like `/history/me`, `/admin/history/all`, and `/history/submissions/:submissionId`) 
 * are defined BEFORE the dynamic route parameter `/:id`. If `/:id` came first, Express 
 * would treat the literal word "history" as a question ID and break route matching.
 */
...
// All routes defined below are protected and require a valid Bearer token in the Authorization header
router.use(protect)
...
router
  .route('/')
  .get(authorize('ADMIN', 'TUTOR'), getQuestions)
  .post(authorize('ADMIN', 'TUTOR'), createQuestion)
...
router
  .route('/:id')
  .get(authorize('ADMIN', 'TUTOR'), getQuestionById)
  .put(authorize('ADMIN', 'TUTOR'), updateQuestion)
  .delete(authorize('ADMIN', 'TUTOR'), deleteQuestion)
