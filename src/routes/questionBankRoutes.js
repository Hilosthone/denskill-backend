 ...
 * @file questionBankRoutes.js
 * @description Express router managing endpoints for question banks, supporting CRUD operations,
 * tutor submissions, admin reviews, bulk question imports, archival/soft deletes, and explicit 
 * question reordering with proper route ordering to prevent parameter collision.
 */
 ...
 *         schema:
 *           type: string
 *         description: Filter question banks by status (e.g. DRAFT, PENDING_REVIEW, APPROVED, ARCHIVED)
 *       - in: query
 *         name: courseId
 *         schema:
 *           type: string
 ...
 *         schema:
 *           type: string
 *         description: Pass 'true' to include archived/soft-deleted question banks in query results
 *       - in: query
 *         name: page
 *         schema:
 *           type: integer
 ...
 *                 items:
 *                   type: object
 *                 example: [{"questionText": "What does CORS stand for?", "difficulty": "EASY", "options": [{"text": "Cross-Origin Resource Sharing", "isCorrect": true}]}]
 *     responses:
 *       200:
 *         description: Import validation results returned
 *       400:
 ...
 * Specific sub-routes containing actions/slugs (such as `/submit`, `/review`, `/import`, and `/reorder`) 
 * MUST be declared BEFORE the generic dynamic parameter route `/:id`. 
 * 
 * Placing `/:id` above these would cause Express to treat words like "reorder" or "submit" 
 * as the variable `:id`, leading to routing crashes, casting failures, or 404 errors.
 */
 ...
 * /api/question-banks/{id}/import:
 *   post:
 *     summary: Bulk sync/overwrite programming questions in a question bank with difficulty and duplicate protection (Restricted to Admins and Tutors)
 *     tags: [Question Banks]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: Questions successfully synced and updated
 *       400:
 *         description: Invalid payload format, option count out of bounds, or duplicate question found
 *       403:
 *         description: Forbidden - Requires Admin or Tutor role
 *       404:
 *         description: Question bank not found
...
/**
 * @swagger
 * /api/question-banks/{id}/reorder:
 *   patch:
 *     summary: Update sequence ordering indices of questions within a question bank (Restricted to Admins and Tutors)
 *     tags: [Question Banks]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *     responses:
 *       200:
 *         description: Question sequencing updated successfully
 *       400:
 *         description: Invalid payload format
 *       403:
 *         description: Forbidden - Insufficient permissions
 ...
 *         description: Question bank not found
 */
router.patch('/:id/reorder', authorize('ADMIN', 'TUTOR'), reorderQuestions)

/**
 * @swagger
 * /api/question-banks/{id}:
 *   get:
 *     summary: Get a single programming question bank by ID with its sorted questions and options
 *     tags: [Question Banks]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         description: Question bank not found
 *   delete:
 *     summary: Soft delete / archive a programming question bank to protect student exam attempt foreign keys (Restricted to Admins and Tutors)
 *     tags: [Question Banks]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: integer
 *         description: ID of the question bank to archive/delete
 *       - in: query
 *         name: hardDelete
 *         schema:
 *           type: string
 *         description: Set to 'true' and authenticate as Admin to perform permanent hard deletion
 *     responses:
 *       200:
 *         description: Question bank archived or permanently deleted successfully
 *       403:
 *         description: Forbidden - Insufficient permissions
 *       404:
 *         description: Question bank not found
